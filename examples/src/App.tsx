...
// 新: ①DOM 見出しは FADE_START→FADE_END でなめらかに消える（スッと）
//     ②粒子は swapFade で静止した字形のままふわっと出現（凝縮したテキストの形）
//     ③timing[0]=hold まで粒子は完全静止 → そこから初めて拡散が始まる
const FADE_START = 0.03; // DOM 見出しのフェード開始進捗
const FADE_END = 0.09; // DOM 見出しのフェード完了進捗（粒子出現 0.078 と少し重ねる）
 ...
 *  - 見出しは FADE_START→FADE_END でなめらかに消え、粒子が swapFade で静止フェードイン。
 */
export function App() {
  ...
  // スクロール進捗に応じて DOM 見出しをなめらかにフェードアウト（スッと消える）。
  // 粒子側は swapFade で静止フェードインするため、瞬時切替の段差が出ない。
  useEffect(() => {
    const onScroll = () => {
