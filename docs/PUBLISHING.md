...
- [x] **リポジトリ置き場所**: **LINNO org = `github.com/linno-inc`** に決定（2026-06-22）。
      （`github.com/linno` は他者が取得済みのため `linno-inc` を採用。`linno-inc` は空き確認済み 2026-06-22）
      `package.json`（repository/homepage/bugs）と README の GIF raw URL は `linno-inc/glyphdust` に設定済み。
- [ ] ★ GitHub org `linno-inc` を作成（**未作成**・凜さんがアカウント操作で実施）。
      作成後、`gh` 認証ユーザー **NOGUCHILin** を org メンバー（repo 作成権限あり）に追加すること。
