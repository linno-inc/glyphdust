...
- [ ] **リポジトリ置き場所**: `NOGUCHILin/glyphdust`（個人）か `LINNO org/glyphdust`（法人）か。
      現状 `package.json` の `repository` / `homepage` / `bugs` と README の GIF raw URL は
      **`NOGUCHILin/glyphdust`** 前提。org にする場合は両方を一括置換する。
- [ ] npm の公開アカウント（個人 or LINNO org スコープ）。現状は無スコープ `glyphdust`（名前は空き確認済み 2026-06-22）。
