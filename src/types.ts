...
export interface GlyphStyle {
   ...
   * 0 で無効（旧来の均一な透明度）。
   * 2026-07-11 品質向上 Phase 1 で追加（提案者: Claude、凜さん承認）。
   */
  alphaVar?: number;
  /**
   ...
   * ポストプロセッシング不要のシェーダ内実装で、パフォーマンス影響は無視できる。
   * 0 で無効。2026-07-11 品質向上 Phase 1 で追加（提案者: Claude、凜さん承認）。
   */
  dof?: number;
  /**
   ...
   * 「風が撫でるように塊（斑）単位で溶け・集まる」有機的な進行になる
   * （Codrops Gommage 2026-01 等の名作と同じノイズマスク設計）。
   * `stagger` が 0 のときは波も無効（分散自体が無いため）。
   * 2026-07-11 品質向上 Phase 2 で追加（提案者: Claude、凜さん承認）。
   */
  wave?: number;
  /**
   * 到着直前の follow-through（余韻）の強さ 0..1。既定はプリセット依存（default 0.35）。
   * キーフレーム到達の少し手前で目標をわずかに行き過ぎ（最大 +6%）、戻って収束する。
   * Disney アニメーション原則の Follow-through / Overlapping action。
   * 補間カーブの端点（t=0/1）は厳密に不変のため、キーフレーム到達点での
   * DOM ピクセル一致・resolve の整列保証は壊れない。reduced-motion では自動無効。
   * 0 で無効（従来の単調な収束）。
   * 2026-07-11 品質向上 Phase 2 で追加（提案者: Claude、凜さん承認）。
   */
  overshoot?: number;
