...
export interface GlyphStyle {
  ...
  burst?: number;
  /**
   * 粒子ごとの透明度ばらつきの強さ 0..1。**既定は全プリセットで 0（opt-in）**。
   * 飛散中の粒に α の個体差（0.45〜1.0）を与え、浮遊感・奥行き感を作る。
   * 整列・字形時は自動で 1（均一）に畳まれ、文字の可読性は変わらない。
   * ただし拡散の途中で粒群の質感が変わって見えるため既定オンは凜さんが却下
   * （2026-07-11「拡散がスムーズじゃない」）。使う場合のみ style で明示指定。
   * 2026-07-11 品質向上 Phase 1 で追加（提案者: Claude）。
   */
  alphaVar?: number;
  /**
   * 擬似被写界深度（DoF）の強さ 0..1。**既定は全プリセットで 0（opt-in）**。
   * フォーカス面（テキストの z=0 平面）から奥・手前に離れた粒ほど、レンズの
   * ボケ円のように「大きく・柔らかく・薄く・わずかに脱彩度」になり、飛散雲に
   * 奥行きの層が生まれる。整列した文字はフォーカス面上にあるため影響ゼロ。
   * ただし移動中の粒のサイズ・ボケが z 位置と連動して毎フレーム変調されるため
   * 明滅（ジッター）として知覚され、既定オンは凜さんが却下（2026-07-11
   * 「拡散がスムーズじゃない」）。使う場合のみ style で明示指定。
   * 2026-07-11 品質向上 Phase 1 で追加（提案者: Claude）。
   */
  dof?: number;
  /**
   * stagger（到達タイミング分散）の空間相関度 0..1。**既定は全プリセットで 0（opt-in）**。
   ...
   * 2026-07-11 凜さん依頼「テキストが粒子にばらけていくように」を受け
   * opt-in ノブとして再導入（提案者: Claude）。
   */
  wave?: number;
  /**
   ...
   *
   * 有効化には optional peer dependency `@react-three/postprocessing` の
   * インストールが必要（未インストールなら警告して bloom 無しで描画継続）。
   * bloom=0 なら postprocessing はロードすらされない（動的 import）。
