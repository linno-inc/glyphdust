...
品質向上フェーズ「美しすぎて魔法」の成果（凜さん目視承認済み 2026-07-12）。

### Added

- **質感・奥行き・溶解波が既定オンに。** `style.alphaVar`（粒の透明度個体差 =
  浮遊感、default 0.55）、`style.dof`（擬似被写界深度 = 奥行きの層、default 0.5）、
  `style.wave`（stagger の空間相関 = 塊単位で溶ける有機的なばらけ、default 0.75）。
  値はプリセット依存。0 を明示指定すれば従来（0.10.0）の見た目に戻せる。
  経緯: 2026-07-11 に一括既定オンを凜さんが却下 → opt-in 化 → 導入・収束の
  スムーズ化完了後、2026-07-12 に凜さんが既定化を承認（提案者: 凜さん）。
- **`style.bloom`（selective bloom = 光の軸）。** きらめき粒・アクセント粒だけが
  内側から光るポスト処理。粒子の軌道・タイミングには一切影響しない。暗背景向けの
  ため既定は `glow` プリセットのみ 0.6。optional peer `@react-three/postprocessing`
  が必要（未インストールなら警告して bloom 無しで描画継続）。モバイル自動オフ。
- **`swapFade` プロップ。** 粒子の出現を瞬時切替からフェードインにする進捗幅
  （既定 0 = 従来どおり）。`timing[0] > 0`（開始キーフレームでの静止保持）と
  組み合わせると「実文字がほどけながら粒子になる」導入が作れる。
- **`swapAt` プロップ。** 粒子が現れる進捗点の上書き（既定は従来の内部値）。
  拡散開始点に合わせると、静止中は実 DOM テキストだけが見え、動き出す瞬間に
  実文字フェードアウト×粒子フェードインの相補クロスフェードが起きる
  （収束側の「動きが終わる瞬間に解決」の正確な鏡像。凜さん設計）。

### Fixed

- **フィナーレの実文字オーバーレイが粒子グリフから右にずれる問題を修正。**
  整列計算が `window.innerWidth` 基準（canvas 実寸との差 = スクロールバー幅）
  だったのを canvas 実寸基準に統一（凜さん実機報告「LINNOの収束のところがずれてる」）。
- **フィナーレの粒子→実文字を逐次からクロスフェードに変更。** 旧: 粒子消失
  （0.90-0.98）→遅れて実文字出現（0.92-1.0）で濃度の谷が段差に見えた。
  ずれ解消でピクセル整列が保証されたため、同窓・相補（合計濃度ほぼ一定）の
  モーフに統一し、滲み→ピントの軽い blur も追加（凜さん「もっとスムーズに
  粒子からテキスト」）。
